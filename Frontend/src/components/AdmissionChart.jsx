...
const AdmissionChart = ({ onCollegeSelect, selectedCollege }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5000/students/admissions');
        if (!response.ok) {
  ...
  return (
    <div className="admission-chart-container">
      <div className="chart-header">
        <h3 className="chart-title">Student Admissions Through Years</h3>
        <p className="chart-subtitle">
          Distribution of students across different colleges
          {selectedCollege && (
